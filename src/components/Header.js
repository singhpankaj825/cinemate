...
export const Header = () => {
  ...
  const activeClass =
    "block py-2 px-3 text-blue text-heading bg-brand rounded md:bg-transparent md:text-fg-brand md:p-0";
  const inActiveClass =
...
return navigate(`search?q=${queryTerm}`);
  ...
  return (
    <header>
      <nav className="bg-neutral-primary w-full z-20 top-0 start-0 border-b border-default dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link
            to="/"
            className="flex items-center space-x-3 rtl:space-x-reverse  dark:text-white"
          >
          ...
          <button data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" 
          onClick={()=>setDarkMode(!darkMode)} type="button" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
            ...
            <div className={`hidden relative md:block`}>
              ...
              <input
                type="text"
                id="input-group-1"
                className="block w-full ps-9 pe-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded focus:ring-brand focus:border-brand px-2.5 shadow-xs placeholder:text-body"
                placeholder="Search"
                autoComplete="off"
                name="search"
